...
function stubForUrl(url) {
  ...
  if (/telemetry|crypto|order/.test(u)) return jsonResponse({ ok: false, error: "local" });
...
try {
  if (globalThis.chrome?.storage?.local?.get) {
    ...
    chrome.storage.local.get = function (keys, cb) {
      ...
      if (ret && typeof ret.then === "function") return ret.then(overlaySettings);
      overlaySettings(ret);
      return ret;
    };
  }
  ...
  if (globalThis.chrome?.storage?.local?.set) {
    ...
    chrome.storage.local.set = function (items, cb) {
      try {
        ...
        if (s && s.last_try_at) globalThis.__et_last_try_at = Number(s.last_try_at) || Date.now();
        else if (items && items.last_try_at)
          globalThis.__et_last_try_at = Number(items.last_try_at) || Date.now();
      } catch {}
  ...
  if (globalThis.chrome?.notifications?.create) {
    ...
    chrome.notifications.create = function (id, options, cb) {
      ...
      if (opts && typeof opts === "object") {
        ...
        if (last > 0 && /aposta feita|aposta recusada/i.test(title + " " + (opts.message || ""))) {
          const sec = ((Date.now() - last) / 1000).toFixed(2);
          const suffix = /aposta feita/i.test(title + " " + (opts.message || ""))
            ? " · feita em " + sec + "s"
            : " · em " + sec + "s";
